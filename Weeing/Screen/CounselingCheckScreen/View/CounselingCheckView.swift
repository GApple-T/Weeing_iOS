...
struct CounselingCheckView: View {
    @State private var toggleColors: [Bool] = [true, true]
    var body: some View {
        ZStack {
                                ...
                                VStack(alignment: .leading) {
                                    ...
                                    HStack {
                                        Text("교과선생님 확인란 : 임성은")
                                            ...
                                            .padding(.leading, 123)
                                        Button(action: {
                                            toggleColors[index].toggle()
                                        }) {
                                            RoundedRectangle(cornerRadius: 3)
                                                .frame(width: 18, height: 18)
                                                .foregroundStyle(toggleColors[index] ? Color.white : Color.P30)
                                                .background(
                                                    RoundedRectangle(cornerRadius: 3)
                                                        .stroke(toggleColors[index] ? Color.gray : Color.P30, lineWidth: 2)
                                                )
                                                .overlay(
